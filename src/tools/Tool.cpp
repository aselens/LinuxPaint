...
DabSpec specFor(StrokeStyle style)
{
    ...
    case StrokeStyle::Airbrush:
        // Размытое облако: край мягкий, а середина за один проход набирает
        // полный цвет. За проход по одному месту ложится около двадцати
        // отпечатков, и 1 − (1 − 0,20)^20 — это уже сплошная заливка.
        return {1.70, 1.00,   0.0, 0.40, 0.20, 0.10, false, false, Grain::None,     true};
    ...
    case StrokeStyle::Crayon:
        // Воск ложится на выступы бумаги и не достаёт до впадин. Цвет на
        // выступах полный: фактуру даёт зерно, а не бледность.
        return {1.00, 1.00,   0.0, 0.55, 0.32, 0.13, false, false, Grain::Paper,    true};
    case StrokeStyle::Marker:
        // Скошенный наконечник. Внутри одного мазка не темнеет: маркер даёт
        // ровный слой, сколько ни води по одному месту.
        return {1.00, 0.55, -30.0, 0.95, 0.80, 0.09, false, false, Grain::None,     false};
    case StrokeStyle::NaturalPencil:
        return {0.75, 1.00,   0.0, 0.45, 0.28, 0.11, false, false, Grain::Graphite, true};
    case StrokeStyle::Watercolour:
        // Широкое пятно с размытым краем.
        return {1.30, 1.00,   0.0, 0.30, 0.30, 0.18, false, false, Grain::Wash,     true};
    }
    return {1.00, 1.00, 0.0, 0.86, 1.00, 0.16, false, false, Grain::None, false};
}
